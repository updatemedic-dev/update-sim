    ...
    return (
      <div className="fixed inset-0 bg-black flex items-center justify-center z-50 p-4">
        <div className="bg-[#111] border border-gray-700 rounded-lg p-6 max-w-lg text-center">
          <h1 className="text-2xl font-bold text-blue-400 mb-2">UPDATE SIM</h1>
          ...
          <div className="my-4 p-3 bg-red-900/30 border border-red-800 rounded text-xs text-red-300">
            ...
            <p>UPDATE SIM es un simulador para entrenamiento médico. NO es un dispositivo médico. NO utilizar para diagnóstico o tratamiento de pacientes reales.</p>
          </div>
          <p className="text-xs text-gray-500 mb-4">Desarrollado por Update Medic — Viña del Mar, Chile</p>
      ...
      {showControls && (
        <div className="shrink-0 border-t border-gray-700 bg-[#0d0d0d]">
          <div className="flex h-[140px]">
            ...
            {/* Column 1: Hide/Show + Scenario buttons */}
            <div className="flex flex-col gap-1 p-1.5 border-r border-gray-800 w-[130px] shrink-0">
              <BottomBtn label="Ocultar FC" onClick={() => useVitalSignsStore.getState().toggleParamVisibility('hr')} />
              <BottomBtn label="Ocultar PA" onClick={() => useVitalSignsStore.getState().toggleParamVisibility('bp')} />
              <BottomBtn label="Ocultar SPO2" onClick={() => useVitalSignsStore.getState().toggleParamVisibility('spo2')} />
              <BottomBtn label="Ocultar CO2" onClick={() => useVitalSignsStore.getState().toggleParamVisibility('etco2')} />
              <BottomBtn label="Ocultar Controles" onClick={() => setShowControls(false)} />
            </div>

            {/* Column 2: Scenarios / Rhythm / Media */}
            <div className="flex flex-col gap-1 p-1.5 border-r border-gray-800 w-[150px] shrink-0">
              <ScenarioRunBtn />
              <BottomBtn label="Escenarios" color="bg-blue-800 hover:bg-blue-700" onClick={() => setShowScenarios(true)} />
              <BottomBtn label="Teclado Ritmos" onClick={() => setShowRhythmPad(!showRhythmPad)} />
              <BottomBtn label="Medicamentos" onClick={() => setShowMeds(!showMeds)} />
              <BottomBtn label="Configuración" onClick={() => setShowSettings(true)} />
            </div>

            {/* Column 3: Pacer controls */}
            <div className="flex flex-col p-1.5 border-r border-gray-800 w-[140px] shrink-0">
              <span className="text-xs font-bold text-white text-center mb-0.5">Pacer</span>
              <div className="flex items-center justify-between">
                ...
                <div className="flex flex-col gap-0.5">
                  <button onClick={() => defib.setPacerRate(defib.pacerRate + 10)}
                    className="w-6 h-5 bg-gray-700 hover:bg-gray-600 rounded text-[10px]">▲</button>
                  <span className="text-[8px] text-gray-500 text-center">Rate</span>
                  <button onClick={() => defib.setPacerRate(defib.pacerRate - 10)}
                    className="w-6 h-5 bg-gray-700 hover:bg-gray-600 rounded text-[10px]">▼</button>
                </div>
              </div>
              <div className="flex items-center justify-between mt-1">
                ...
                <div className="flex flex-col gap-0.5">
                  <button onClick={() => defib.setPacerCurrent(defib.pacerCurrent + 5)}
                    className="w-6 h-5 bg-gray-700 hover:bg-gray-600 rounded text-[10px]">▲</button>
                  <span className="text-[8px] text-gray-500 text-center">mA</span>
                  <button onClick={() => defib.setPacerCurrent(defib.pacerCurrent - 5)}
                    className="w-6 h-5 bg-gray-700 hover:bg-gray-600 rounded text-[10px]">▼</button>
                </div>
              </div>
            ...
            {/* Column 4: Pacer ON + NIBP + CPR + Sync */}
            <div className="flex flex-col gap-1 p-1.5 border-r border-gray-800 w-[120px] shrink-0">
              <button onClick={() => defib.togglePacer()}
                className={`flex-1 rounded font-bold text-xs ${defib.pacerOn ? 'bg-cyan-700 animate-pulse text-white' : 'bg-gray-800 hover:bg-gray-700 text-gray-300'}`}>
              ...
              <button onClick={() => {
                ...
                className={`flex-1 rounded font-bold text-xs ${vitals.cprActive ? 'bg-orange-600 animate-pulse' : 'bg-gray-800 hover:bg-gray-700 text-gray-300'}`}>
                C.P.R.
              </button>
              <button onClick={() => defib.toggleSync()}
                className={`flex-1 rounded font-bold text-xs ${defib.syncMode ? 'bg-green-700 text-white' : 'bg-gray-800 hover:bg-gray-700 text-gray-300'}`}>
                SYNC {defib.syncMode ? '✓' : ''}
              </button>
            </div>

            {/* Column 5: Temperature + Respiration */}
            <div className="flex flex-col p-1.5 border-r border-gray-800 w-[120px] shrink-0">
            ...
            {/* Column 6: Energy Select + Charge + Shock */}
            <div className="flex flex-col p-1.5 flex-1 min-w-[180px]">
              ...
              <div className="flex items-center justify-center gap-2 my-1">
                <button onClick={() => defib.decreaseEnergy()}
                  className="w-8 h-8 bg-gray-700 hover:bg-gray-600 rounded font-bold text-lg text-white">▼</button>
              ...
              <div className="flex gap-1 mt-1">
                <button onClick={() => {
                  ...
                  className={`flex-1 py-2 rounded font-bold text-sm ${defib.isCharging ? 'bg-yellow-700 animate-pulse' : defib.isCharged ? 'bg-yellow-600' : 'bg-red-700 hover:bg-red-600'} text-white disabled:opacity-60`}>
                ...
                <button onClick={() => {
                  ...
                  className={`flex-1 py-2 rounded font-bold text-sm ${defib.isCharged ? 'bg-yellow-500 hover:bg-yellow-400 text-black animate-pulse' : 'bg-gray-800 text-gray-500'}`}>
                  ⚡ Shock
                </button>
              </div>
              <button onClick={() => defib.disarm()}
                className="mt-1 py-1 bg-cyan-800 hover:bg-cyan-700 rounded text-xs font-bold text-white">
                Disarm
              </button>
            </div>
          </div>
...
function BottomBtn({ label, onClick, color }: { label: string; onClick?: () => void; color?: string }) {
  return (
    <button
      onClick={onClick}
      className={`flex-1 px-2 rounded text-[11px] font-medium text-gray-200 ${color ?? 'bg-gray-800 hover:bg-gray-700'}`}
    >
      {label}
    </button>
  );
...
function MedsOverlay({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/80 z-50 flex items-end justify-center pb-[160px]" onClick={onClose}>
      <div className="bg-[#111] border border-gray-700 rounded-lg p-3 max-w-xl w-full" onClick={(e) => e.stopPropagation()}>
        <div className="flex justify-between items-center mb-2">
        ...
        <div className="flex flex-wrap gap-0.5 mb-2">
          {Object.values(MedicationCategory).map((cat) => (
            <button key={cat} onClick={() => setSelectedCat(cat)}
              className={`px-2 py-0.5 rounded text-[10px] ${selectedCat === cat ? 'bg-purple-700 text-white' : 'bg-gray-800 text-gray-400'}`}>
              {cat}
            </button>
          ))}
        </div>
        <div className="grid grid-cols-3 gap-1 max-h-48 overflow-y-auto">
          {filtered.map((med) => (
            <button key={med.id} onClick={() => handleAdmin(med.id)}
              className="px-2 py-1.5 rounded text-[10px] text-left text-white"
