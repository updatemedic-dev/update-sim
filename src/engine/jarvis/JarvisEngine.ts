...
const CLINICAL_TAG = '[VALIDAR CON EXPERTO CLÍNICO]';

function tagClinical(message: string): string {
  return message.includes(CLINICAL_TAG) ? message : `${CLINICAL_TAG} ${message}`;
}
...
export function analyzeSimulation(ctx: SimulationContext): JarvisAnalysis {
  ...
  return {
    ...
    alerts: alerts.map(tagClinical),
    suggestions: suggestions.map(tagClinical),
    status,
  };
}
...
export function getRhythmDescription(rhythm: CardiacRhythm, lang: 'es' | 'en'): string {
  ...
  if (desc) return tagClinical(es ? desc.es : desc.en);

  return tagClinical(es ? `${def.nameEs}. FC objetivo: ${def.hrRange[0]}-${def.hrRange[1]} lpm.`
